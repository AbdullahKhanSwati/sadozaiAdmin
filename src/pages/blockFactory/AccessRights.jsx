...
export default function AccessRights() {
  ...
  const { roles, employeeCount, deleteRoles } = useBlockFactory();
    ...
    <div className="max-w-[1200px] mx-auto">
      <Card>
        <div className="flex items-center gap-4 p-5">
          <PrimaryBtn onClick={() => navigate('/block-factory/employees/access/new')}>+ Add role</PrimaryBtn>
          {selected.length > 0 && (
        ...
        <table className="w-full text-sm min-w-[520px]">
          ...
          <tbody>
            {pageItems.map((r) => (
              <tr key={r.id} onClick={() => navigate(`/block-factory/employees/access/${r.id}`)} className="border-t border-slate-100 hover:bg-slate-50/60 cursor-pointer">
