...
export default function RoleForm() {
  ...
  const { id } = useParams();
  const { roles, saveRole, deleteRole, ready } = useBlockFactory();
  ...
  const onSave = async () => {
    ...
    try {
      ...
      navigate('/block-factory/employees/access');
    } catch (e) { window.alert(e?.message || 'Could not save the role.'); }
  };

  const onDelete = async () => {
    if (!window.confirm('Delete this role?')) return;
    try { await deleteRole(existing.id); navigate('/block-factory/employees/access'); }
    ...
    <div className="max-w-[760px] mx-auto pb-24">
          ...
          <div className="space-y-3">
            <label className="flex items-start gap-3 cursor-pointer" onClick={() => setAccess('both')}>
              ...
              <div>
                ...
                <div className="text-xs text-ink-400">Admin — full access to the dashboard and the Block Factory app.</div>
              </div>
            </label>
            <label className="flex items-start gap-3 cursor-pointer" onClick={() => setAccess('pos')}>
              ...
              <div>
                ...
                <div className="text-xs text-ink-400">Staff — can access the Block Factory app only.</div>
              </div>
            </label>
          </div>
        </div>
      ...
      <div className="flex items-center justify-between mt-4 px-2">
        ...
        <div className="flex gap-3">
          <GhostBtn onClick={() => navigate('/block-factory/employees/access')}>Cancel</GhostBtn>
