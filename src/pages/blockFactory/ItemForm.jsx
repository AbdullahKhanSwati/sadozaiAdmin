...
import { uploadItemImage } from '../../lib/supabaseBlockFactory.js';
import { EditGate } from './formGate.jsx';

export default function ItemForm() {
  const navigate = useNavigate();
  const { id } = useParams();
  const { items, categories, modifiers, saveItem, deleteItem, ready } = useBlockFactory();

  const existing = items.find((i) => i.id === id);
  const [form, setForm] = useState(() =>
    existing || {
      name: '', categoryId: '', description: '', availableForSale: true,
      soldBy: 'Each', price: '', sku: String(10001 + items.length),
  ...
  const onSave = async () => {
    ...
    try {
      await saveItem({
        // Blank price is stored as NULL, not 0 — it means "no default, the rate
        // is typed when the item is added to a ticket in the app".
        ...form,
        price: String(form.price).trim() === '' ? null : Number(form.price) || 0,
        image: rep === 'image' ? form.image : (form.image || ''), variants: cleanVariants,
      });
      navigate('/block-factory/items/list');
    } catch (e) {
  ...
  const onDelete = async () => {
    ...
    try { await deleteItem(existing.id); navigate('/block-factory/items/list'); }
  ...
  return (
    <div className="max-w-[900px] mx-auto pb-24">
      ...
      <Card className="p-6 sm:p-8 mb-4">
        ...
        <div className="mt-6">
          <textarea value={form.description} onChange={(e) => set({ description: e.target.value })} placeholder="Description" rows={2} className="w-full border border-slate-200 rounded-md p-3 text-sm text-ink-700 placeholder:text-slate-400 focus:outline-none focus:border-bf-400" />
        </div>

        <label className="flex items-center gap-3 mt-6 cursor-pointer" onClick={() => set({ availableForSale: !form.availableForSale })}>
          <span className={['w-6 h-6 rounded flex items-center justify-center border-2', form.availableForSale ? 'bg-bf-500 border-bf-500' : 'border-slate-300'].join(' ')}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-6 mt-8">
          <Field
            ...
            hint="Only a default. The rate is entered when the item is added to a ticket in the app — leave blank to type it fresh every sale."
          >
      ...
      <Card className="p-6 sm:p-8 mb-4">
        ...
        <button onClick={addVariant} className="flex items-center gap-2 text-bf-600 font-bold text-sm uppercase tracking-wide">
          <Plus className="w-5 h-5 rounded-full border-2 border-bf-600 p-0.5" /> Add variants
        </button>
      </Card>
      ...
      <Card className="p-6 sm:p-8 mb-4">
        ...
        {rep === 'color' ? (
          ...
          <label className="w-40 h-40 bg-slate-50 border-2 border-dashed border-slate-200 rounded-md flex flex-col items-center justify-center text-slate-400 gap-2 cursor-pointer overflow-hidden relative">
            {uploading ? (
              <Loader2 className="w-8 h-8 animate-spin text-bf-500" />
            ) : form.image ? (
              <>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1" />
        <GhostBtn onClick={() => navigate('/block-factory/items/list')}>Cancel</GhostBtn>
