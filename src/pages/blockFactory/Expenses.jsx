...
import BlockFactoryExpenseCategoriesDialog from '../../components/dialogs/BlockFactoryExpenseCategoriesDialog.jsx';
...
export default function Expenses() {
  const { expenses: rows, expenseCategories, reloadExpenses } = useBlockFactory();
  ...
  const onExport = () => downloadCsv(`block-factory-expenses-${csvDate()}.csv`, [
    ...
    <div className="max-w-[1100px] mx-auto">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3 mb-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button onClick={openNew} className="inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-bf-600 text-white text-sm font-semibold hover:bg-bf-700">
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-4 mb-4 flex flex-wrap items-end gap-4">
        ...
        {(from || to || category) && (
          <button onClick={() => { setFrom(''); setTo(''); setCategory(''); }} className="text-sm font-semibold text-bf-600 hover:text-bf-700 py-2">Clear</button>
        )}
        ...
        {loading ? (
          ...
          <div key={e.id} className="grid grid-cols-[1fr_1fr_auto_80px] gap-2 px-4 py-3 items-center border-b border-slate-50 last:border-b-0 hover:bg-slate-50/60">
            ...
            <div className="flex justify-end gap-1">
              <button onClick={() => openEdit(e)} className="text-slate-400 hover:text-bf-600 p-1"><Pencil className="w-4 h-4" /></button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 animate-fade-in" onClick={() => setForm(null)}>
          <div className="bg-white rounded-lg shadow-2xl w-full max-w-md" onClick={(ev) => ev.stopPropagation()}>
            ...
            <div className="p-5 space-y-4">
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  ...
                  <button onClick={() => setCatsOpen(true)} className="text-xs font-semibold text-bf-600 hover:text-bf-700">Manage</button>
                </div>
            ...
            <div className="flex justify-end gap-3 px-5 py-4 border-t border-slate-100">
              ...
              <button onClick={save} disabled={saving} className="px-4 py-2 rounded-lg bg-bf-600 text-white text-sm font-semibold hover:bg-bf-700 disabled:opacity-50">
      ...
      <BlockFactoryExpenseCategoriesDialog open={catsOpen} onClose={() => setCatsOpen(false)} />
    </div>
  );
}
