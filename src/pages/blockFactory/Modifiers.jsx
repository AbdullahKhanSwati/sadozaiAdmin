...
export default function Modifiers() {
  ...
  const { modifiers, deleteModifiers } = useBlockFactory();
    ...
    <div className="max-w-[1000px] mx-auto">
      <Card>
        <div className="flex items-center gap-4 p-5">
          <PrimaryBtn onClick={() => navigate('/block-factory/items/modifiers/new')}>+ Add modifier</PrimaryBtn>
          {selected.length > 0 && (
        ...
        <table className="w-full text-sm border-t border-slate-100">
          ...
          <tbody>
            {modifiers.map((m) => (
              <tr key={m.id} onClick={() => navigate(`/block-factory/items/modifiers/${m.id}`)} className="border-t border-slate-100 hover:bg-slate-50/60 cursor-pointer">
