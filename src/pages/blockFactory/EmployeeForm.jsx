...
import { createBlockFactoryLogin, adminSetStaffPassword, adminSetStaffRole, adminDeleteStaff } from '../../lib/supabaseBlockFactory.js';
...
export default function EmployeeForm() {
  ...
  const { id } = useParams();
  const { employees, roles, saveEmployee, deleteEmployee, role, ready } = useBlockFactory();
  ...
  const accessText =
    selectedRole?.access === 'pos'
      ? 'POS — can sign in to the Block Factory app only'
  ...
  const onDelete = async () => {
    ...
    try {
      ...
      navigate('/block-factory/employees/list');
    } catch (e) {
    ...
    try {
      if (!existing) {
        ...
        if (form.email.trim() && password) {
          ...
          const { user, alreadyExisted } = await createBlockFactoryLogin({
  ...
  return (
    <div className="max-w-[820px] mx-auto pb-24">
      <Card className="p-6 sm:p-8">
        ...
        <div className="mt-4 flex items-start gap-2 rounded-md bg-bf-50 border border-bf-100 px-3.5 py-2.5 text-xs text-bf-700">
      ...
      <div className="flex items-center justify-between mt-4 px-2">
        ...
        <div className="flex gap-3">
          <GhostBtn onClick={() => navigate('/block-factory/employees/list')}>Cancel</GhostBtn>
