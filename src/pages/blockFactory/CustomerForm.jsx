...
export default function CustomerForm() {
  ...
  const { id } = useParams();
  const { customers, saveCustomer, deleteCustomer, reports, ready } = useBlockFactory();
  ...
  const onSave = async () => {
    ...
    try {
      ...
      navigate('/block-factory/customers');
    } catch (e) { window.alert(e?.message || 'Could not save the customer.'); }
  };

  const onDelete = async () => {
    if (!window.confirm('Delete this customer?')) return;
    try { await deleteCustomer(existing.id); navigate('/block-factory/customers'); }
  ...
  return (
    <div className="max-w-[900px] mx-auto pb-24">
      <Card className="p-6 sm:p-8">
        ...
        <textarea value={form.note} onChange={(e) => set({ note: e.target.value })} rows={3} placeholder="Note" className="w-full border border-slate-200 rounded-md p-3 text-sm text-ink-700 placeholder:text-slate-400 focus:outline-none focus:border-bf-400" />
      </Card>

      {existing && (
        <Card className="mt-4 overflow-hidden">
          <div className="flex items-center gap-2 px-6 py-4 border-b border-slate-100">
            <Receipt className="w-5 h-5 text-bf-600" />
      ...
      <div className="flex items-center justify-between mt-4 px-2">
        ...
        <div className="flex gap-3">
          <GhostBtn onClick={() => navigate('/block-factory/customers')}>Cancel</GhostBtn>
