...
export default function EmployeeList() {
  ...
  const { employees, roleName, deleteEmployees } = useBlockFactory();
    ...
    <div className="max-w-[1200px] mx-auto">
      ...
      <Card>
        <div className="flex items-center gap-4 p-5">
          <PrimaryBtn onClick={() => navigate('/block-factory/employees/new')}>+ Add employee</PrimaryBtn>
          {selected.length > 0 && (
            <button onClick={async () => { if (!window.confirm(`Delete ${selected.length} employee(s)?`)) return; try { await deleteEmployees(selected); setSelected([]); } catch (e) { window.alert(e?.message || 'Delete failed.'); } }} className="flex items-center gap-1.5 text-sm font-bold uppercase tracking-wide text-rose-500 hover:text-rose-600">
        ...
        <table className="w-full text-sm min-w-[560px]">
          ...
          <tbody>
            {pageItems.map((e) => (
              <tr key={e.id} onClick={() => navigate(`/block-factory/employees/${e.id}`)} className="border-t border-slate-100 hover:bg-slate-50/60 cursor-pointer">
