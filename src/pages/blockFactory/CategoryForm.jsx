...
export default function CategoryForm() {
  ...
  const { id } = useParams();
  const { categories, saveCategory, deleteCategory, ready } = useBlockFactory();
  ...
  const onSave = async () => {
    ...
    try {
      ...
      navigate('/block-factory/items/categories');
    } catch (e) { window.alert(e?.message || 'Could not save the category.'); }
  };

  const onDelete = async () => {
    if (!window.confirm('Delete this category?')) return;
    try { await deleteCategory(existing.id); navigate('/block-factory/items/categories'); }
  ...
  return (
    <div className="max-w-[900px] mx-auto">
      ...
      <div className="flex items-center justify-end gap-6 mt-4 px-2">
        ...
        <TextBtn onClick={() => navigate('/block-factory/items/categories')}>Cancel</TextBtn>
