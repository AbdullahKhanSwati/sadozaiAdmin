...
export default function DiscountForm() {
  ...
  const { id } = useParams();
  const { discounts, saveDiscount, deleteDiscount, ready } = useBlockFactory();
  ...
  const onSave = async () => {
    ...
    try {
      ...
      navigate('/block-factory/items/discounts');
    } catch (e) { window.alert(e?.message || 'Could not save the discount.'); }
  };

  const onDelete = async () => {
    if (!window.confirm('Delete this discount?')) return;
    try { await deleteDiscount(existing.id); navigate('/block-factory/items/discounts'); }
    ...
    <div className="max-w-[720px] mx-auto pb-24">
      <Card className="p-6 sm:p-8">
        ...
        <label className="flex items-center gap-3 mt-6 cursor-pointer" onClick={() => setVariable((v) => !v)}>
          <span className={['w-5 h-5 rounded flex items-center justify-center border-2', variable ? 'bg-bf-500 border-bf-500' : 'border-slate-300'].join(' ')}>
      ...
      <div className="flex items-center justify-between mt-4 px-2">
        ...
        <div className="flex gap-3">
          <GhostBtn onClick={() => navigate('/block-factory/items/discounts')}>Cancel</GhostBtn>
