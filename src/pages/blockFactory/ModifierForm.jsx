...
export default function ModifierForm() {
  ...
  const { id } = useParams();
  const { modifiers, saveModifier, deleteModifier, ready } = useBlockFactory();
  ...
  const onSave = async () => {
    ...
    try {
      ...
      navigate('/block-factory/items/modifiers');
    } catch (e) { window.alert(e?.message || 'Could not save the modifier.'); }
  };

  const onDelete = async () => {
    if (!window.confirm('Delete this modifier?')) return;
    try { await deleteModifier(existing.id); navigate('/block-factory/items/modifiers'); }
    ...
    <div className="max-w-[720px] mx-auto pb-24">
      <Card className="p-6 sm:p-10">
        <div className="flex justify-center mb-8">
          <span className="w-20 h-20 rounded-full bg-bf-500 text-white flex items-center justify-center">
          ...
          {options.map((o, i) => (
            <div key={i} className="flex items-end gap-4">
              ...
              <div className="flex-1">
                <div className="text-xs text-bf-600 mb-1">Option name</div>
        ...
        <button onClick={addOpt} className="flex items-center gap-2 text-bf-600 font-bold text-sm uppercase tracking-wide mt-8">
      ...
      <div className="flex items-center justify-between mt-4 px-2">
        ...
        <div className="flex gap-3">
          <GhostBtn onClick={() => navigate('/block-factory/items/modifiers')}>Cancel</GhostBtn>
