...
import { ReportToolbar, Panel, usePagination, TablePagination } from './bfUi.jsx';
...
export default function Receipts() {
  const { reports, cancelReceipt, restoreReceipt } = useBlockFactory();
  ...
  const exportReceipts = () => downloadCsv(`block-factory-receipts-${csvDate()}.csv`, [
    ...
    { label: 'Bill reference', value: (r) => r.billRef || '' },
  ...
  const exportLines = () => {
    ...
    downloadCsv(`block-factory-receipt-lines-${csvDate()}.csv`, [
      { label: 'Bill reference', value: (l) => l.billRef || '' },
      { label: 'Receipt no.', value: 'no' },
      ...
      { label: 'Unit of measure', value: (l) => l.unitLabel || '' },
            ...
            <tbody>
              {pageItems.map((r) => (
                <tr key={r.id || r.no} onClick={() => setOpenId(r.id)} className="border-t border-slate-100 hover:bg-slate-50/60 cursor-pointer">
                  <td className="px-5 py-4 font-bold text-ink-800">{r.billRef || '—'}</td>
              ...
              {rows.length === 0 && (
                <tr><td colSpan={10} className="px-5 py-10 text-center text-ink-400">No receipts found.</td></tr>
              )}
            </tbody>
          </table>
        </div>
          ...
          <div className="bg-white rounded-lg shadow-2xl w-full max-w-md max-h-[85vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-5">
              ...
              <div className="text-sm text-ink-600 space-y-1 mb-4">
                ...
                {detail.billRef && <div>Bill reference: <span className="text-ink-800 font-medium">{detail.billRef}</span></div>}
                  ...
                  <div key={i} className="flex justify-between py-2 border-b border-slate-50 last:border-b-0">
                    <div className="min-w-0 pr-3">
                      ...
                      <div className="text-xs text-ink-400">
                        {l.qty}{l.unitLabel ? ` ${l.unitLabel}` : ''} × {rs(l.unit)}
                      </div>
                      {(l.mods || []).map((m, mi) => (
