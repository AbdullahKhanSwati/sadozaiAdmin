...
import { supabaseBlockFactory, deleteOwnAccount } from '../../lib/supabaseBlockFactory.js';
...
export default function Account() {
  ...
  const [form, setForm] = useState(() => ({
    businessName: settings?.businessName || 'Block Factory',
  ...
  const onSave = async () => {
    ...
    try {
      ...
      if (editEmail && email && email !== session?.email) {
        const { error } = await supabaseBlockFactory.auth.updateUser({ email });
        if (error) throw error;
      }
      if (editPass && password) {
        const { error } = await supabaseBlockFactory.auth.updateUser({ password });
        if (error) throw error;
      }
      navigate(-1);
    } catch (e) {
      ...
      <Card className="px-6 sm:px-8 pt-6">
        ...
        <div className="mt-10 border-t border-slate-100 py-6 flex items-start justify-between gap-4">
          <div>
            ...
            <div className="text-sm text-ink-400 mt-0.5 max-w-sm">You can permanently delete your Block Factory account and all its data</div>
          </div>
          <button onClick={onDelete} className="text-bf-600 font-bold uppercase tracking-wide text-sm hover:text-bf-700 shrink-0">Delete</button>
        </div>
      </Card>
