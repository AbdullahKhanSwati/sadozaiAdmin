...
export default function Categories() {
  ...
  const { categories, itemCount, deleteCategories } = useBlockFactory();
    ...
    <div className="max-w-[1000px] mx-auto">
      <Card>
        <div className="flex items-center gap-4 p-5">
          <PrimaryBtn onClick={() => navigate('/block-factory/items/categories/new')}>+ Add category</PrimaryBtn>
          {selected.length > 0 && (
            <button onClick={async () => { if (!window.confirm(`Delete ${selected.length} category(s)?`)) return; try { await deleteCategories(selected); setSelected([]); } catch (e) { window.alert(e?.message || 'Delete failed.'); } }} className="flex items-center gap-1.5 text-sm font-bold uppercase tracking-wide text-rose-500 hover:text-rose-600">
        ...
        <table className="w-full text-sm border-t border-slate-100">
          ...
          <tbody>
            {categories.map((c) => (
              <tr key={c.id} onClick={() => navigate(`/block-factory/items/categories/${c.id}`)} className="border-t border-slate-100 hover:bg-slate-50/60 cursor-pointer">
