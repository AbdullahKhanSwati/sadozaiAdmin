...
import { Panel, usePagination, TablePagination } from './bfUi.jsx';
...
export default function ItemList() {
  ...
  const { items, categories, categoryName, saveItem, deleteItems } = useBlockFactory();
  ...
  const onExport = () => downloadCsv(`block-factory-items-${csvDate()}.csv`,
    [
      ...
      { label: 'Default price', value: (r) => (r.price ?? '') },
  ...
  const onImport = async (e) => {
    ...
    for (const r of rows) {
      ...
      await saveItem({
        ...
        // Blank stays blank — the rate is then typed at sale time in the app.
        price: (() => {
          const v = r['Default price'] ?? r.Price ?? r.price;
          return String(v ?? '').trim() === '' ? null : Number(v) || 0;
        })(),
    ...
    <div className="max-w-[1400px] mx-auto">
      <Card>
        ...
        <div className="flex flex-wrap items-center gap-4 p-5">
          <PrimaryBtn onClick={() => navigate('/block-factory/items/new')}>+ Add item</PrimaryBtn>
          ...
          <label className="flex flex-col text-xs text-ink-400">
            Category
            <select value={cat} onChange={(e) => setCat(e.target.value)} className="mt-1 border-b border-slate-300 bg-transparent py-1 text-sm text-ink-700 focus:outline-none focus:border-bf-500 min-w-[140px]">
          ...
          <label className="flex flex-col text-xs text-ink-400">
            Stock alert
            <select className="mt-1 border-b border-slate-300 bg-transparent py-1 text-sm text-ink-700 focus:outline-none focus:border-bf-500 min-w-[120px]">
          ...
          <div className="relative">
            ...
            <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search" className="pl-9 pr-3 py-2 text-sm bg-slate-50 border border-slate-200 rounded-md focus:outline-none focus:ring-2 focus:ring-bf-500/30 focus:border-bf-400 w-44" />
          </div>
        </div>
              ...
              <tr className="text-ink-500">
                ...
                <th className="text-left font-medium px-2 py-3">
                  <button onClick={() => setSortDir((d) => (d === 'asc' ? 'desc' : 'asc'))} className="inline-flex items-center gap-1 hover:text-ink-700">
                    Item name
                    {sortDir === 'asc' ? <ArrowUp className="w-3.5 h-3.5 text-bf-600" /> : <ArrowDown className="w-3.5 h-3.5 text-bf-600" />}
                  </button>
                </th>
                <th className="text-left font-medium px-5 py-3">Category</th>
                <th className="text-right font-medium px-5 py-3">Default price</th>
                <th className="text-right font-medium px-5 py-3">In stock</th>
              </tr>
            </thead>
            <tbody>
              {pageItems.map((i) => (
                <tr key={i.id} onClick={() => navigate(`/block-factory/items/${i.id}`)} className="border-t border-slate-100 hover:bg-slate-50/60 cursor-pointer">
                  ...
                  {/* Blank means "priced at sale time" — not free. */}
                  <td className="px-5 py-4 text-right text-ink-700">
                    {i.price ? rs(i.price) : <span className="text-ink-400 italic">Set at sale</span>}
                  </td>
                  <td className="px-5 py-4 text-right text-ink-400">—</td>
                </tr>
              ))}
              {pageItems.length === 0 && (
                <tr><td colSpan={5} className="px-5 py-10 text-center text-ink-400">No items found.</td></tr>
              )}
            </tbody>
          </table>
        </div>
