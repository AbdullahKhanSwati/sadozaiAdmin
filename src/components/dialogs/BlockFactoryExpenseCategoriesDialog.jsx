...
/**
 * Manage the Block Factory expense categories stored in `expense_categories`.
 ...
 */
export default function BlockFactoryExpenseCategoriesDialog({ open, onClose }) {
  const {
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-start justify-between mb-5">
          <div>
            ...
            <p className="text-xs text-ink-500 mt-0.5">Used here and on the Block Factory app's Add-expense screen.</p>
          </div>
              ...
              <div key={c.id} className="flex items-center gap-2 rounded-xl border border-slate-200 p-2.5">
                {isEditing ? (
                  <>
                    ...
                    />
                    <button onClick={() => saveEdit(c)} disabled={busy} className="p-2 rounded-lg bg-bf-600 text-white hover:bg-bf-700 disabled:opacity-50">
                  ...
                  <>
                    ...
                    <button onClick={() => { setEditingId(c.id); setEditName(c.name); }} className="p-2 rounded-lg text-slate-400 hover:text-bf-600 hover:bg-slate-50">
        ...
        <div className="mt-4 flex gap-2">
          ...
          />
          <button onClick={add} disabled={busy || !newName.trim()} className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg bg-bf-600 text-white text-sm font-semibold hover:bg-bf-700 disabled:opacity-50">
