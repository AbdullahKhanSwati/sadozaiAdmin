...
import {
  Menu, BarChart3, ShoppingBasket, Contact, Users, Boxes, Wallet,
  Settings as SettingsIcon, LogOut, ChevronDown,
} from 'lucide-react';
import { useAuth } from '../context/AuthContext.jsx';

// ---- Nav model -----------------------------------------------------------
const B = '/block-factory';
...
const REPORT_CHILDREN = [
  ...
  { to: `${B}/reports/receivables`,        label: 'Receivables' },
...
const NAV = [
  ...
  { id: 'expenses',  label: 'Expenses',  icon: Wallet,         color: '#E53935', to: `${B}/expenses` },
...
function titleFor(pathname) {
  ...
  if (/\/customers\/[^/]+\/statement$/.test(pathname)) return 'Customer statement';
...
export default function BlockFactoryLayout() {
  ...
  return (
    <div className="h-screen flex flex-col bg-slate-100 text-ink-800">
      ...
      <header className="h-14 bg-bf-600 text-white flex items-center px-3 sm:px-4 gap-2 sm:gap-3 shadow-md shrink-0 z-30">
...
function Sidebar({ variant, expanded, openId, setOpenId, flyoutId, setFlyoutId, pathname, session, onLogout, onNavigate }) {
  ...
  const email = session?.email || 'blockfactory@gmail.com';
      ...
      <div className="relative border-b border-slate-100">
        ...
        {profileOpen && (
          <div className={['absolute z-50 bg-white rounded-md border border-slate-200 shadow-pop py-1 w-44 animate-fade-in', expanded ? 'left-4 top-[60px]' : 'left-full top-2 ml-1'].join(' ')}>
            <button onClick={() => { setProfileOpen(false); closeAndGo(() => navigate('/block-factory/account')); }} className="block w-full text-left px-4 py-2.5 text-sm text-ink-700 hover:bg-slate-50">
              Account
            </button>
    ...
    <NavLink
      ...
      className={({ isActive }) =>
        [
          ...
          isActive ? 'text-bf-700 bg-bf-50 font-semibold' : 'text-ink-600 hover:bg-slate-50',
        ].join(' ')
      }
    >
      {({ isActive }) => (
        <>
          {isActive && <span className="absolute left-0 top-0 bottom-0 w-1 bg-bf-600" />}
...
function GroupItem({ item, expanded, open, setOpen, active, flyoutOpen, setFlyout, flyoutRef, onNavigate }) {
  ...
  const rowTone = active ? 'text-bf-700 bg-bf-50 font-semibold' : 'text-ink-600 hover:bg-slate-50';
  ...
  return (
    <div className="relative">
      ...
      >
        {active && <span className="absolute left-0 top-0 bottom-0 w-1 bg-bf-600" />}
        <Icon className="w-6 h-6 shrink-0" style={{ color: item.color }} />
        {expanded && (
          <>
        ...
        <div className="pb-1">
          {children.map((c) => (
            <NavLink
              ...
              onClick={onNavigate}
              className={({ isActive }) =>
                ['block pl-14 pr-4 py-2 text-[13px] transition', isActive ? 'text-bf-700 font-semibold bg-bf-50' : 'text-ink-500 hover:bg-slate-50'].join(' ')
              }
            >
              {c.label}
            </NavLink>
        ...
        <div ref={flyoutRef} className="absolute left-full top-0 ml-1 w-56 bg-white rounded-md border border-slate-200 shadow-pop z-50 py-1 animate-fade-in">
          ...
          {children.map((c) => (
            <NavLink
              ...
              className={({ isActive }) =>
                ['block px-4 py-2.5 text-sm transition', isActive ? 'text-bf-700 font-semibold bg-bf-50' : 'text-ink-600 hover:bg-slate-50'].join(' ')
              }
            >
              {c.label}
            </NavLink>
